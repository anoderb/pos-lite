...
export default function OwnerProdukPage() {
  ...
  return (
    <div className="max-w-[430px] mx-auto space-y-4 pb-24 text-[#10233E]">
      {/* Promo Summary Card */}
      <section className="relative overflow-hidden rounded-[22px] p-4 bg-gradient-to-br from-[#E8FAF0] via-white to-[#FFF8D9] shadow-[0_2px_10px_rgba(16,35,62,.05)]">
        <div className="relative z-10 max-w-[58%]">
          <h1 className="text-base font-semibold leading-6">Katalog & Multi-Harga</h1>
          <p className="text-[11px] font-normal text-[#68758A] leading-4 mt-1">Kelola produk, harga eceran/grosir, dan ambang stok minimum.</p>
        </div>
        <img src="/assets/tokiva-dashboard/img-produk1.png" alt="Katalog produk" className="absolute right-1 bottom-1 w-[46%] h-[86%] object-contain object-right-bottom" />
        <div className="relative z-10 grid grid-cols-3 gap-1.5 mt-3 max-w-[62%]">
          <div className="flex items-center gap-1.5">
            <span className="w-7 h-7 rounded-lg bg-[#0CAF60]/10 text-[#0CAF60] flex items-center justify-center shrink-0"><Package className="w-3.5 h-3.5" /></span>
            <div className="min-w-0"><p className="text-[13px] font-medium leading-4">{stats.totalProduk}</p><p className="text-[9px] font-normal text-[#68758A] truncate">Produk</p></div>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-7 h-7 rounded-lg bg-[#F3EEFF] text-violet-600 flex items-center justify-center shrink-0"><LayoutGrid className="w-3.5 h-3.5" /></span>
            <div className="min-w-0"><p className="text-[13px] font-medium leading-4">{stats.kategori}</p><p className="text-[9px] font-normal text-[#68758A] truncate">Kategori</p></div>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-7 h-7 rounded-lg bg-[#FFF0F0] text-[#F05B61] flex items-center justify-center shrink-0"><AlertTriangle className="w-3.5 h-3.5" /></span>
            <div className="min-w-0"><p className="text-[13px] font-medium leading-4">{stats.stokRendah}</p><p className="text-[9px] font-normal text-[#68758A] truncate">Stok Rendah</p></div>
          </div>
        </div>
      </section>
      ...
      <div className="flex gap-2">
        <button
          ...
          className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 bg-[#0CAF60] text-white rounded-xl text-xs font-medium shadow-sm hover:bg-[#087A4B] active:scale-[0.98] transition-all"
        >
        ...
        <button
          ...
          className="px-3 py-2.5 bg-white border border-gray-200 text-[#68758A] rounded-xl text-xs font-medium flex items-center gap-1.5 shadow-sm hover:bg-gray-50 active:scale-[0.98] transition-all"
        >
            ...
            {filteredProduk.slice((page - 1) * 20, page * 20).map((p) => {
              ...
              return (
                <div
                  key={p.id}
                  className={cn(
                    'bg-white rounded-[16px] p-3 shadow-sm border flex items-center gap-3 transition-all',
                    isKritis ? 'border-[#F5C6C9] bg-[#FFF8F8]' : 'border-gray-50'
                  )}
                >
                  <div className="relative shrink-0">
                    <ProdukThumb nama={p.nama} img={p.foto_url || p.img || p.fotos?.[0]} className="w-12 h-12 rounded-xl" />
      ...
      <div className={cn(
        'fixed inset-x-0 bottom-0 z-40 bg-white rounded-t-[24px] shadow-2xl flex flex-col h-full transition-transform duration-300 ease-out',
        ...
        <form onSubmit={handleSave} className="flex-1 min-h-0 flex flex-col">
          <div className="flex-1 overflow-y-auto hide-scrollbar px-4 py-3 space-y-3 pb-6">
                {/* AI Banner */}
